...
public class FilialController {
    ...
    @GetMapping("/filtros")
    public ResponseEntity<List<FilialResponse>> getFiliaisFiltros(
            @RequestParam String nome,
            @RequestParam(required = false) Integer anosServico
    ) {
        List<FilialResponse> response = filialService.buscaNomeAnoServico(nome, anosServico);
